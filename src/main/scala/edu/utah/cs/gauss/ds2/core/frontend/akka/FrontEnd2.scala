...
object FrontEnd2 extends App {
  ...
  def addToClassPath(s: String) = {
    val u: URL = new File(s).toURI.toURL
//    val urlClassLoader: URLClassLoader = ClassLoader.getSystemClassLoader.asInstanceOf[URLClassLoader]
    val urlClassLoader: URLClassLoader = new URLClassLoader(Array(u), ClassLoader.getSystemClassLoader)
...
//          cl.getClass.getClassLoader.getURLs.foreach(println)
...
class ObjectInputStreamWithCustomClassLoader2(fileInputStream: FileInputStream) extends ObjectInputStream(fileInputStream) {
