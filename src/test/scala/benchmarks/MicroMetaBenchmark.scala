...
import benchmarks.erroneosregister.ErrDistRegisterDS2System
import benchmarks.register._
...
object MicroMetaBenchmark {
   ...
   */
  def getDistributedSystemInstance(system: Systems.Value, numAgents: Int = 5, log: Boolean = false): (DistributedSystem, File) = {
    system match {
      case Systems.DISTRIBUTED_REGISTER         =>
        DistRegisterDS2System.getInstance(numOfAgents = numAgents, log = log)
      case Systems.ERR_DISTRIBUTED_REGISTER         =>
        ErrDistRegisterDS2System.getInstance(numOfAgents = numAgents, log = log)
      case Systems.ANOTHER_DISTRIBUTED_REGISTER =>
        SampleDistributedSystems.sampleDistributedRegister_FINAL_VERSION_SIMPLIFIED("./err.dist.reg.harness.1.txt", numOfAgents = numAgents, log = log)
//        SampleDistributedSystems.sampleDistributedRegister_FINAL_VERSION("./err.dist.reg.harness.1.txt", numOfAgents = numAgents, log = log)
      case Systems.ZAB =>
        benchmarks.zab.ZabDS2System.getInstance(numAgents,"./zab.harness.txt", log)
      case Systems.PAXOS =>
        benchmarks.paxos.PaxosDS2System.getInstance(numAgents,"./paxos.harness.txt", log)
      case Systems.OPEN_CHORD =>
        benchmarks.openchord.service.ChordDS2System.getInstance(numAgents,"./open.chord.harness.txt", log)
//      case Systems.RAFT => ???
  ...
  def doBenchmark(alg: Algorithms.Value, sys: Systems.Value,
                 ...
                 numAgents: Int = 5,
                  ...
                  numOfWrites: Int = 1,
                  numOfReads: Int = 2,
                  log: Boolean = false): Seq[BenchmarkResults] = {
    require(howManyTimes >= 1, "Do you want to run this scheduler ZERO or less times? fine! :(")

    val (system, file) = getDistributedSystemInstance(sys, numAgents, log)
    ...
    (1 to howManyTimes) map { _: Int =>
      val sch = alg match {
        ...
        case ROUND_ROBIN =>
          new RoundRobinScheduler(file.getPath, system, Int.MaxValue, benchmark = true, numSchedulesLimit = numOfSchedulesLimit)
        case EXHAUSTIVE =>
          new ExhaustiveDFSScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case DELAY_BOUNDED =>
          new DelayBoundedScheduler(file.getPath, system, true, log, delays, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case DPOR =>
          new DporScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case TRANS_DPOR =>
          new TransDporScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case IRED =>
          new IRedScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case LIVIOLA =>
          new LiViolaScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case DP_IRED =>
          new DPIRedScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
        case DP_LIVIOLA =>
          new DPLiViolaScheduler(file.getPath, system, true, log, Int.MaxValue, historySizeLimit, numOfSchedulesLimit)
      }

      if (alg != RANDOM && alg != ROUND_ROBIN) { // all DFS schedulers
        ...
        if(theScheduler.isInstanceOf[DPIRedScheduler] ||
          theScheduler.isInstanceOf[DPLiViolaScheduler]){
          // TD, IR, LV
          // init only enabled set
          theScheduler.currentState.setEnabledSet(theScheduler.schedule.toSet)
          theScheduler.currentState.asInstanceOf[DporState].setBacktrackingSet(theScheduler.currentState.getEnabledSet)
        }
        else if(theScheduler.isInstanceOf[TransDporScheduler] ||
      ...
      else { // only ROUND_ROBIN scheduler
        val scheduler = sch.asInstanceOf[RoundRobinScheduler]
        scheduler.schedule = scheduler.schedule.filter { r => scheduler.isModificationCall(r._1) }.take(numOfWrites) ++
          scheduler.schedule.filterNot { r => scheduler.isModificationCall(r._1) }.take(numOfReads)
        scheduler.enabledSet = scheduler.schedule.toSet
        sch.benchmarkStatistics.harness = sch.schedule.toList
...
//    avg.firstBuggyHistIdx = br1.firstBuggyHistIdx //Math.floorDiv(br1.firstBuggyHistIdx,br2.firstBuggyHistIdx)
   ...
   */
  def multiResultsString(rowOfTuplesOfResults: Seq[(BenchmarkResults, BenchmarkResults)]): String = {
    val strBuilder = new StringBuilder
    // The lines in a benchmark row:
    // #UH
    strBuilder.append(rowOfTuplesOfResults.map { x => s"""${x._1.numUniqueHist} & ${x._2.numUniqueHist}""" }.mkString(" & ") + """\\ \cline{2-16}"""+"\n")
    // #NL
    strBuilder.append(rowOfTuplesOfResults.map { x => s"""${x._1.numBuggyHistories} & ${x._2.numBuggyHistories}""" }.mkString(" & ") + """\\ \cline{2-16}\n"""+"\n")
    // #IH
    strBuilder.append(rowOfTuplesOfResults.map { x => s"""${x._1.numIncompleteHistories} & ${x._2.numIncompleteHistories}""" }.mkString(" & ") + """\\ \cline{2-16}\n"""+"\n")
    // NL/S
    strBuilder.append(rowOfTuplesOfResults.map { x => s"""${x._1.numBuggyHistories / x._1.schedules.size} & ${x._2.numBuggyHistories / x._2.schedules.size}""" }.mkString(" & ") + """\\ \cline{2-16}\n"""+"\n")
    // TS
    strBuilder.append(rowOfTuplesOfResults.map { x =>
      ...
      s"""${d1.toMinutes + ":" + "%02d" format d1.toSeconds % 60} & ${d2.toMinutes + ":" + "%02d" format d2.toSeconds % 60}"""
    }.mkString(" & ") + """\\ \cline{2-16}\n"""+"\n")
    // ST
    strBuilder.append(rowOfTuplesOfResults.map { x =>
    ...
    strBuilder.append(rowOfTuplesOfResults.map { x =>
      ...
      s"""${d1.toMinutes + ":" + "%02d" format d1.toSeconds % 60} & ${d2.toMinutes + ":" + "%02d" format  d2.toSeconds % 60}"""}.mkString(" & ") + """\\ \cline{2-16}\n"""+"\n")
