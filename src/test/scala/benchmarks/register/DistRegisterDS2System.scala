...
object DistRegisterDS2System {
  def getInstance(numOfAgents: Int = 2, harnessFilePath: String = "./dist.reg.harness.txt", log: Boolean = false): (DistributedSystem, File) = {
    val systemName = "Distributed Register"
  ...
  private def createAgent(name: String, peersCount: Int, leader: Boolean = false, log: Boolean = false): Agent = new DistRegDS2Agent(name, peersCount, leader, log)
