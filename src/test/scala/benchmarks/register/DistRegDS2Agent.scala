...
case class WriteReplica(k: String, value: Any, client: Agent, uuid: String, round: Int = 0) extends Message("WriteReplica", Seq(k, value, client, uuid, round))

case class WriteReplicaAck(k: String, value: Any, client: Agent, uuid: String, round: Int = 0) extends Message("WriteReplicaAck", Seq(k, value, client, uuid, round))
...
case class ReadReplica(k: String, client: Agent, uuid: String, round: Int = 0) extends Message("ReadReplica", Seq(k, client, uuid, round))

case class ReadReplicaAck(k: String, value: Any, client: Agent, uuid: String, round: Int = 0) extends Message("ReadReplicaAck", Seq(k, value, client, uuid, round))

case class ReadAck(k: String, value: Any) extends Message("ReadAck", Seq(k, value))



class DistRegDS2Agent(name: String, val peersCount: Int, val leader: Boolean = false, val log: Boolean = false) extends Agent(name) {
  //-------------------------------------------------------------------------------------------
  //                                     Init and Decls
  //-------------------------------------------------------------------------------------------
  // declarations
   ...
   */
  def getSystemAgents: Set[Agent] = ds.agents.filterNot(_.name.startsWith("IRed"))

  def getPeers: Set[Agent] = getSystemAgents - this

  def getLeader: Agent = getPeers.filter { case x: DistRegDS2Agent => x.leader }.head
  ...
  val writeAction: Action = new Action + Statement { (m: Message, a: Agent) =>
    ...
    if (leader) {
      ...
      if (m.payload.size == 2) // direct from client
        a.localState[WRITES_TRACKER_TYPE](WRITES_TRACKER)(m.sender) = MajorityTracker(peersCount = this.peersCount, value = value)
      else if (m.payload.size == 3) // forwarded from another peer
        a.localState[WRITES_TRACKER_TYPE](WRITES_TRACKER)(m.payload[Agent](2)) = MajorityTracker(peersCount = this.peersCount, value = value)
  ...
  val readAction: Action = new Action + Statement { (m: Message, a: Agent) =>
    ...
    if (leader) {
      ...
      if (m.payload.size == 1) // direct from client
        a.localState[READS_TRACKER_TYPE](READS_TRACKER)(m.sender) = MajorityTracker(peersCount = this.peersCount, value = thisValue)
      else if (m.payload.size == 2) // forwarded from other peers
        a.localState[READS_TRACKER_TYPE](READS_TRACKER)(m.payload[Agent](1)) = MajorityTracker(peersCount = this.peersCount, value = thisValue)

      val uuid = UUID.randomUUID().toString.takeRight(4)
      for (peer <- getPeers) {
        var msg = new Message("whatever will get replaced")
        //        DONE fix the ReadReplica to have the client in the payload if originally forwarded by the other peers
