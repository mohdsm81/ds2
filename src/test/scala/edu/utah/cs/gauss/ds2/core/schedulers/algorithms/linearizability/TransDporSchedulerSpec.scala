...
import java.io.File
class TransDporSchedulerSpec extends MyTestSpecs{
  test("test - experimenting") {
    ...
    val sch = new TransDporScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
  ...
  ignore("test - the paper example") {
    ...
    val sample: (DistributedSystem, File) = SampleDistributedSystems.transDporExample("./harness.1.txt")
    val sch = new TransDporScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 1000)
    ...
    assert(sch.benchmarkStatistics.schedules.size == 13)
