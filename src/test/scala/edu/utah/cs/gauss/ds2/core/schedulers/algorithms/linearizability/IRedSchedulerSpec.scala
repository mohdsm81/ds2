...
class IRedSchedulerSpec extends MyTestSpecs{
  test("test - experimenting") {

    val sample: (DistributedSystem, File) =
      SampleDistributedSystems.sampleLinearizableNAgentsRegisterSTRICT_limited_R_Retries_AND_NO_cache_TAINTED("./harness.ired.1.txt", 5, log=false)
    val sch = new IRedScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
  ...
  ignore("test - the paper example") {
    ...
    val sample: (DistributedSystem, File) = SampleDistributedSystems.transDporExample("./harness.ired.2.txt")
    val sch = new IRedScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
...
//    println(sch.benchmarkStatistics.toString)
  ...
  ignore("test - the paper example 2") {
    ...
    val sample: (DistributedSystem, File) = SampleDistributedSystems.transDporExample("./harness.ired.3.txt")
    val sch = new IRedScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
...
//    println(sch.benchmarkStatistics.toString)
  ...
  test("test - err dist reg") {
    ...
    val sample: (DistributedSystem, File) =
      SampleDistributedSystems.sampleLinearizableNAgentsRegisterSTRICT_limited_R_Retries_AND_NO_cache_TAINTED_ERRONEOUS("./harness.ired.4.txt", 5, log=false)
    val sch = new IRedScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
