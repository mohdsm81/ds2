 ...
 */
//@Ignore
class TestEchoTestBedAskWithGetTimeoutCopy extends MyTestSpecs {
  info( """=================================================================""" )
  info(
    """This test suite is an integration test suite. It starts with a
  ...
  info( """=================================================================""" )
  val myDS = TestBeds.echoServerWithAskInstanceWithTimedGetCopy
  ...
  test( "DistributedSystem and Schedulers know each other" ) {
    ...
    assert (scheduler.consumeQ.isEmpty)
  }
  ...
  test( "Client can't access its future to know if the server reacted, and client is blocked, due to server not " +
          "resolving it" ) {
