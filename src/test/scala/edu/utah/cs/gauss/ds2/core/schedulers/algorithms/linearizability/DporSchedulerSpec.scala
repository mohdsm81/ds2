...
import edu.utah.cs.gauss.ds2.core.schedulers.algorithms.dpor.{DporScheduler, DporState}

class DporSchedulerSpec extends MyTestSpecs{
  test("test - experimenting") {

    val sample: (DistributedSystem, File) =
      SampleDistributedSystems.sampleLinearizableNAgentsRegisterSTRICT_limited_R_Retries_AND_NO_cache_TAINTED("./harness.dp.1.txt", 5, log=false)
    val sch = new DporScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
  ...
  ignore("test - the paper example") {
    ...
    val sample: (DistributedSystem, File) = SampleDistributedSystems.transDporExample("./harness.dp.2.txt")
    val sch = new DporScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
    ...
    sch.currentState.asInstanceOf[DporState].setBacktrackingSet(sch.currentState.getEnabledSet)
...
//    sch.dependencySet = sch.dependencySet ++ sch.currentState.getEnabledSet.map{x => (x -> x)}
    ...
    assert(sch.benchmarkStatistics.schedules.size == 24)
...
//    println(sch.benchmarkStatistics.toString)
  ...
  ignore("test - the paper example 2") {

    val sample: (DistributedSystem, File) = SampleDistributedSystems.transDporExample("./harness.dp.3.txt")
    val sch = new DporScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
    ...
    val w1 = new Message(registryAddress)
    w1.sender = master
    val w1_2 = new Message(registryAddress)
    w1.sender = master
    val w2 = new Message(registryAddress)
    w2.sender = master
...
//    println(sch.benchmarkStatistics.toString)
  ...
  test("test - err dist reg") {
    ...
    val sample: (DistributedSystem, File) =
      SampleDistributedSystems.sampleLinearizableNAgentsRegisterSTRICT_limited_R_Retries_AND_NO_cache_TAINTED_ERRONEOUS("./harness.dp.4.txt", 5, log=false)
    val sch = new DporScheduler(sample._2.getPath, sample._1, benchmark = true, log=false, numOfSchedulesLimit = 10)
